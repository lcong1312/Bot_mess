...
module.exports = {
  config: {
    ...
    category: "love",
    description: {
      vi: "Tạo ảnh vua của bạn",
      en: "Create your king image"
    },
    guide: {
      vi: "{pn} @tag | reply | UID",
  ...
  onStart: async function ({ api, usersData, event, args }) {const senderID = event.senderID;
    let target;
    ...
    if (!target)
      return api.sendMessage(
        "❌ Tag, reply hoặc nhập UID để chọn vua của bạn!",
        event.threadID,
        event.messageID
      );
